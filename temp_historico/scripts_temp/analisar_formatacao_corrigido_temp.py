...
def analisar_formatacao_artigos():
    ...
    try:
        ...
        print("Buscando databases disponíveis...")
        for db in databases["results"]:
            if "title" in db and len(db["title"]) > 0:
                nome_db = db["title"][0]["text"]["content"]
                print(f"   Database encontrado: {nome_db}")
                
                if "gestao" in nome_db.lower() or "editorial" in nome_db.lower() or "educacional" in nome_db.lower():
                    database_id = db["id"]
        ...
        if not database_id:
            print("ERRO: Nenhum database de gestão educacional encontrado")
            print("Usando o primeiro database disponível...")
            if databases["results"]:
                database_id = databases["results"][0]["id"]
                database_nome = databases["results"][0]["title"][0]["text"]["content"] if databases["results"][0]["title"] else "Database sem nome"
                print(f"   Database selecionado: {database_nome}")
            else:
                print("ERRO: Nenhum database encontrado")
                return False
